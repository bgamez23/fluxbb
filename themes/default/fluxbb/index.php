...
if ($pun_config['o_feed_type'] == '1')
	$page_head = array('feed' => '<link rel="alternate" type="application/rss+xml" href="?module=fluxbb&action=extern&act=feed&amp;type=rss" title="'.$lang_common['RSS active topics feed'].'" />');
else if ($pun_config['o_feed_type'] == '2')
	$page_head = array('feed' => '<link rel="alternate" type="application/atom+xml" href="?module=fluxbb&action=extern&act=feed&amp;type=atom" title="'.$lang_common['Atom active topics feed'].'" />');
...
<?php
	...
	if (!$pun_user['is_guest'] && $cur_forum['last_post'] > $pun_user['last_visit'] && (empty($tracked_topics['forums'][$cur_forum['fid']]) || $cur_forum['last_post'] > $tracked_topics['forums'][$cur_forum['fid']]))
	{
		...
		foreach ($new_topics[$cur_forum['fid']] as $check_topic_id => $check_last_post)
		{
			...
			{
				$item_status .= ' inew';
				$forum_field_new = '<span class="newtext">[ <a href="?module=fluxbb&action=search&act=show_new&amp;fid='.$cur_forum['fid'].'">'.$lang_common['New posts'].'</a> ]</span>';
	...
	{
		$forum_field = '<h3><a href="?module=fluxbb&action=viewforum&id='.$cur_forum['fid'].'">'.pun_htmlspecialchars($cur_forum['forum_name']).'</a>'.(!empty($forum_field_new) ? ' '.$forum_field_new : '').'</h3>';
	...
	if ($cur_forum['last_post'] != '')
		$last_post = '<a href="?module=fluxbb&action=viewtopic&pid='.$cur_forum['last_post_id'].'#p'.$cur_forum['last_post_id'].'">'.format_time($cur_forum['last_post']).'</a> <span class="byuser">'.$lang_common['by'].' '.pun_htmlspecialchars($cur_forum['last_poster']).'</span>';
	...
	if ($cur_forum['moderators'] != '')
	{
		...
		foreach ($mods_array as $mod_username => $mod_id)
		{
			if ($pun_user['g_view_users'] == '1')
				$moderators[] = '<a href="?module=fluxbb&action=profile&id='.$mod_id.'">'.pun_htmlspecialchars($mod_username).'</a>';
			else
...
if ($pun_user['g_view_users'] == '1')
	$stats['newest_user'] = '<a href="?module=fluxbb&action=profile&id='.$stats['last_user']['id'].'">'.pun_htmlspecialchars($stats['last_user']['username']).'</a>';
else
...
if ($pun_config['o_users_online'] == '1')
{
	...
	while ($pun_user_online = $db->fetch_assoc($result))
	{
		...
		{
			if ($pun_user['g_view_users'] == '1')
				$users[] = "\n\t\t\t\t".'<dd><a href="?module=fluxbb&action=profile&id='.$pun_user_online['user_id'].'">'.pun_htmlspecialchars($pun_user_online['ident']).'</a>';
			else
