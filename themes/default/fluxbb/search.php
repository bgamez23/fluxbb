...
if ($params->get('act') || $params->get('search_id'))
{
	$action = ($params->get('act')) ? $params->get('act') : null;
		
	$forums = ($params->get('forums')) ? (is_array($params->get('forums')) ? $params->get('forums') : array_filter(explode(',', $params->get('forums')))) : ($params->get('forums') ? array($params->get('forums')) : array());
	...
	{
		$search_id = intval($params->get('search_id'));
		if ($search_id < 1)
	...
	{
		$keywords = ($params->get('keywords')) ? utf8_strtolower(pun_trim($params->get('keywords'))) : null;
		$author = ($params->get('author')) ? utf8_strtolower(pun_trim($params->get('author'))) : null;
		...
		$show_as = ($params->get('show_as') && $params->get('show_as') == 'topics') ? 'topics' : 'posts';
		$sort_by = ($params->get('sort_by')) ? intval($params->get('sort_by')) : 0;
		$search_in = (!$params->get('search_in') || $params->get('search_in') == '0') ? 0 : (($params->get('search_in') == '1') ? 1 : -1);
	}
	// If it's a user search (by ID)
	else if ($action == 'show_user_posts' || $action == 'show_user_topics' || $action == 'show_subscriptions')
	{
		if( $params->get('user_id') )
			$user_id = ($params->get('user_id')) ? intval($params->get('user_id')) : $pun_user['id'];
		else
			$user_id = ($params->get('user_id')) ? intval($params->get('user_id')) : $pun_user['id'];
		...
		$interval = $params->get('value') ? intval($params->get('value')) : 86400;
	...
	else
	{
		...
		if (!empty($author) || !empty($keywords))
		{
			...
			if ($author && $keywords)
			{
				...
				$search_type = array('both', array($keywords, pun_trim($params->get('author'))), implode(',', $forums), $search_in);
			}
			else if ($keywords)
			{
			...
			else
			{
				...
				$search_type = array('author', pun_trim($params->get('author')), implode(',', $forums), $search_in);
			}
		...
		else if ($action == 'show_new' || $action == 'show_recent' || $action == 'show_replies' || $action == 'show_user_posts' || $action == 'show_user_topics' || $action == 'show_subscriptions' || $action == 'show_unanswered')
		{
			...
			if ($action == 'show_new')
			{
				...
				$result = $db->query('SELECT t.id FROM '.$db->prefix.'topics AS t LEFT JOIN '.$db->prefix.'forum_perms AS fp ON (fp.forum_id=t.forum_id AND fp.group_id='.$pun_user['g_id'].') WHERE (fp.read_forum IS NULL OR fp.read_forum=1) AND t.last_post>'.$pun_user['last_visit'].' AND t.moved_to IS NULL'.($params->get('fid') ? ' AND t.forum_id='.intval($params->get('fid')) : '').' ORDER BY t.last_post DESC') or error('Unable to fetch topic list', __FILE__, __LINE__, $db->error());
			...
			{
				$result = $db->query('SELECT t.id FROM '.$db->prefix.'topics AS t LEFT JOIN '.$db->prefix.'forum_perms AS fp ON (fp.forum_id=t.forum_id AND fp.group_id='.$pun_user['g_id'].') WHERE (fp.read_forum IS NULL OR fp.read_forum=1) AND t.last_post>'.(time() - $interval).' AND t.moved_to IS NULL'.($params->get('fid') ? ' AND t.forum_id='.intval($params->get('fid')) : '').' ORDER BY t.last_post DESC') or error('Unable to fetch topic list', __FILE__, __LINE__, $db->error());
				$num_hits = $db->num_rows($result);
		...
		if ($search_type[0] != 'act')
		{
			$db->end_transaction();
			$db->close();

			// Redirect the user to the cached result page
			header('Location: ?module=fluxbb&action=search&search_id='.$search_id);
			exit;
		}
	}
	...
	if (!$pun_user['is_guest'] && $search_type[0] == 'act' && $search_type[1] == 'show_new')
		$forum_actions[] = '<a href="?module=fluxbb&action=misc&act=markread">'.$lang_common['Mark all as read'].'</a>';
	...
	if (!empty($search_ids))
	{
		...
		// Determine the topic or post offset (based on $params->get('p'))
		...
		$paging_links = '<span class="pages-label">'.$lang_common['Pages'].' </span>'.paginate($num_pages, $p, '?module=fluxbb&action=search&search_id='.$search_id);
		...
		if ($search_type[0] == 'act')
		{
			if ($search_type[1] == 'show_user_topics')
				$crumbs_text['search_type'] = '<a href="?module=fluxbb&action=search&act=show_user_topics&amp;user_id='.$search_type[2].'">'.sprintf($lang_search['Quick search show_user_topics'], pun_htmlspecialchars($search_set[0]['poster'])).'</a>';
			else if ($search_type[1] == 'show_user_posts')
				$crumbs_text['search_type'] = '<a href="?module=fluxbb&action=search&act=show_user_posts&amp;user_id='.$search_type[2].'">'.sprintf($lang_search['Quick search show_user_posts'], pun_htmlspecialchars($search_set[0]['pposter'])).'</a>';
			else if ($search_type[1] == 'show_subscriptions')
			{
				...
				$crumbs_text['search_type'] = '<a href="?module=fluxbb&action=search&act=show_subscriptions&amp;user_id='.$subscriber_id.'">'.sprintf($lang_search['Quick search show_subscriptions'], pun_htmlspecialchars($subscriber_name)).'</a>';
			}
			else
				$crumbs_text['search_type'] = '<a href="?module=fluxbb&action=search&act='.$search_type[1].'">'.$lang_search['Quick search '.$search_type[1]].'</a>';
		}
		else
		{
			$keywords = $author = '';
			...
			$crumbs_text['search_type'] = '<a href="?module=fluxbb&action=search&act=search&amp;keywords='.urlencode($keywords).'&amp;author='.urlencode($author).'&amp;forums='.$search_type[2].'&amp;search_in='.$search_type[3].'&amp;sort_by='.$sort_by.'&amp;sort_dir='.$sort_dir.'&amp;show_as='.$show_as.'">'.$crumbs_text['search_type'].'</a>';
		}
...
<div class="linkst">
	<div class="inbox crumbsplus">
		<ul class="crumbs">
			<li><a href="?module=fluxbb"><?php echo $lang_common['Index'] ?></a></li>
			<li><span>»&#160;</span><a href="?module=fluxbb&action=search"><?php echo $crumbs_text['show_as'] ?></a></li>
...
<?php
		...
		{
			$forum = '<a href="?module=fluxbb&action=viewforum&id='.$cur_search['forum_id'].'">'.pun_htmlspecialchars($cur_search['forum_name']).'</a>';
			...
			if ($show_as == 'posts')
			{
				...
				if ($cur_search['poster_id'] > 1)
				{
					if ($pun_user['g_view_users'] == '1')
						$pposter = '<strong><a href="?module=fluxbb&action=profile&id='.$cur_search['poster_id'].'">'.$pposter.'</a></strong>';
					else
...
<div class="blockpost<?php echo ($post_count % 2 == 0) ? ' roweven' : ' rowodd' ?><?php if ($cur_search['pid'] == $cur_search['first_post_id']) echo ' firstpost' ?><?php if ($post_count == 1) echo ' blockpost1' ?><?php if ($item_status != '') echo ' '.$item_status ?>">
	<h2><span><span class="conr">#<?php echo ($start_from + $post_count) ?></span> <span><?php if ($cur_search['pid'] != $cur_search['first_post_id']) echo $lang_topic['Re'].' ' ?><?php echo $forum ?></span> <span>»&#160;<a href="?module=fluxbb&action=viewtopic&id=<?php echo $cur_search['tid'] ?>"><?php echo pun_htmlspecialchars($cur_search['subject']) ?></a></span> <span>»&#160;<a href="?module=fluxbb&action=viewtopic&pid=<?php echo $cur_search['pid'].'#p'.$cur_search['pid'] ?>"><?php echo format_time($cur_search['pposted']) ?></a></span></span></h2>
	<div class="box">
		<div class="inbox">
			<div class="postbody">
				<div class="postleft">
		...
		<div class="inbox">
			<div class="postfoot clearb">
				<div class="postfootright">
					<ul>
						<li><span><a href="?module=fluxbb&action=viewtopic&id=<?php echo $cur_search['tid'] ?>"><?php echo $lang_search['Go to topic'] ?></a></span></li>
						<li><span><a href="?module=fluxbb&action=viewtopic&pid=<?php echo $cur_search['pid'].'#p'.$cur_search['pid'] ?>"><?php echo $lang_search['Go to post'] ?></a></span></li>
					</ul>
				</div>
			</div>
		</div>
...
<?php
			...
			else
			{
				...
				$subject = '<a href="?module=fluxbb&action=viewtopic&id='.$cur_search['tid'].'">'.pun_htmlspecialchars($cur_search['subject']).'</a> <span class="byuser">'.$lang_common['by'].' '.pun_htmlspecialchars($cur_search['poster']).'</span>';
				...
				if (!$pun_user['is_guest'] && $cur_search['last_post'] > $pun_user['last_visit'] && (!isset($tracked_topics['topics'][$cur_search['tid']]) || $tracked_topics['topics'][$cur_search['tid']] < $cur_search['last_post']) && (!isset($tracked_topics['forums'][$cur_search['forum_id']]) || $tracked_topics['forums'][$cur_search['forum_id']] < $cur_search['last_post']))
				{
					...
					$subject_new_posts = '<span class="newtext">[ <a href="?module=fluxbb&action=viewtopic&id='.$cur_search['tid'].'&amp;act=new" title="'.$lang_common['New posts info'].'">'.$lang_common['New posts'].'</a> ]</span>';
				}
				else
				...
				if ($num_pages_topic > 1)
					$subject_multipage = '<span class="pagestext">[ '.paginate($num_pages_topic, -1, '?module=fluxbb&action=viewtopic&id='.$cur_search['tid']).' ]</span>';
				else
...
?>
				<tr class="<?php echo $item_status ?>">
					...
					<td class="tcr"><?php echo '<a href="?module=fluxbb&action=viewtopic&pid='.$cur_search['last_post_id'].'#p'.$cur_search['last_post_id'].'">'.format_time($cur_search['last_post']).'</a> <span class="byuser">'.$lang_common['by'].' '.pun_htmlspecialchars($cur_search['last_poster']) ?></span></td>
				</tr>
<?php
...
<div id="searchform" class="blockform">
	...
	<div class="box">
		<form id="search" method="post" action="?module=fluxbb&action=search">
			<div class="inform">
				<fieldset>
