...
else if (isset($_GET['email']))
{
	...
	if (!isset($redirect_url))
		$redirect_url = '?module=fluxbb&action=profile&id='.$recipient_id;
	else if (preg_match('%?module=viewtopic&pid=(\d+)$%', $redirect_url, $matches))
...
<div id="emailform" class="blockform">
	...
	<div class="box">
		<form id="email" method="post" action="?module=fluxbb&action=misc&email=<?php echo $recipient_id ?>" onsubmit="this.submit.disabled=true;if(process_form(this)){return true;}else{this.submit.disabled=false;return false;}">
			<div class="inform">
				<fieldset>
	...
	if (isset($_POST['form_sent']))
	{
		...
		if ($pun_config['o_report_method'] == '1' || $pun_config['o_report_method'] == '2')
		{
			...
			if ($pun_config['o_mailing_list'] != '')
			{
				...
				$mail_message = str_replace('<post_url>', '?module=fluxbb&action=viewtopic&pid='.$post_id.'#p'.$post_id, $mail_message);
...
<div class="linkst">
	<div class="inbox">
		<ul class="crumbs">
			...
			<li><span>»&#160;</span><a href="?module=fluxbb&action=viewforum&id=<?php echo $cur_post['fid'] ?>"><?php echo pun_htmlspecialchars($cur_post['forum_name']) ?></a></li>
			<li><span>»&#160;</span><a href="?module=fluxbb&action=viewtopic&pid=<?php echo $post_id ?>#p<?php echo $post_id ?>"><?php echo pun_htmlspecialchars($cur_post['subject']) ?></a></li>
...
<div id="reportform" class="blockform">
	...
	<div class="box">
		<form id="report" method="post" action="?module=fluxbb&action=misc&report=<?php echo $post_id ?>" onsubmit="this.submit.disabled=true;if(process_form(this)){return true;}else{this.submit.disabled=false;return false;}">
			<div class="inform">
				<fieldset>
