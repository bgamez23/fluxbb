...
else if ($action == 'new')
{
	if (!$pun_user['is_guest'])
	{
		...
		{
			header('Location: ?module=fluxbb&action=viewtopic&pid='.$first_new_post_id.'#p'.$first_new_post_id);
			exit;
		}
	}

	// If there is no new post, we go to the last post
	header('Location: ?module=fluxbb&action=viewtopic&id='.$id.'&act=last');
	exit;
}
...
else if ($action == 'last')
{
	...
	if ($last_post_id)
	{
		header('Location: ?module=fluxbb&action=viewtopic&pid='.$last_post_id.'#p'.$last_post_id);
		exit;
	}
}
...
if ($cur_topic['closed'] == '0')
{
	if (($cur_topic['post_replies'] == '' && $pun_user['g_post_replies'] == '1') || $cur_topic['post_replies'] == '1' || $is_admmod)
		$post_link = "\t\t\t".'<p class="postlink conr"><a href="?module=fluxbb&action=post&tid='.$id.'">'.$lang_topic['Post reply'].'</a></p>'."\n";
	else
		$post_link = '';
}
else
{
	...
	if ($is_admmod)
		$post_link .= ' / <a href="?module=fluxbb&action=post&tid='.$id.'">'.$lang_topic['Post reply'].'</a>';
...
$paging_links = '<span class="pages-label">'.$lang_common['Pages'].' </span>'.paginate($num_pages, $p, '?module=fluxbb&action=viewtopic&id='.$id);
...
if (!$pun_user['is_guest'] && $pun_config['o_topic_subscriptions'] == '1')
{
	if ($cur_topic['is_subscribed'])
		...
		$subscraction = "\t\t".'<p class="subscribelink clearb" style="top:33px;"><span>'.$lang_topic['Is subscribed'].' - </span><a href="?module=fluxbb&action=misc&act=unsubscribe&amp;tid='.$id.'">'.$lang_topic['Unsubscribe'].'</a></p>'."\n";
	else
		$subscraction = "\t\t".'<p class="subscribelink clearb" style="top:33px;"><a href="?module=fluxbb&action=misc&act=subscribe&amp;tid='.$id.'">'.$lang_topic['Subscribe'].'</a></p>'."\n";
}
else
	$subscraction = '';

if ($pun_config['o_feed_type'] == '1')
	$page_head = array('feed' => '<link rel="alternate" type="application/rss+xml" href="?module=fluxbb&action=extern&act=feed&amp;tid='.$id.'&amp;type=rss" title="'.$lang_common['RSS topic feed'].'" />');
else if ($pun_config['o_feed_type'] == '2')
	$page_head = array('feed' => '<link rel="alternate" type="application/atom+xml" href="?module=fluxbb&action=extern&act=feed&amp;tid='.$id.'&amp;type=atom" title="'.$lang_common['Atom topic feed'].'" />');
...
<div class="linkst">
	<div class="inbox crumbsplus">
		<ul class="crumbs">
			...
			<li><span>»&#160;</span><a href="?module=fluxbb&action=viewforum&id=<?php echo $cur_topic['forum_id'] ?>"><?php echo pun_htmlspecialchars($cur_topic['forum_name']) ?></a></li>
			<li><span>»&#160;</span><strong><a href="?module=fluxbb&action=viewtopic&id=<?php echo $id ?>"><?php echo pun_htmlspecialchars($cur_topic['subject']) ?></a></strong></li>
		</ul>
		<div class="pagepost">
...
while ($cur_post = $db->fetch_assoc($result))
{
	...
	if ($cur_post['poster_id'] > 1)
	{
		if ($pun_user['g_view_users'] == '1')
			$username = '<a href="?module=fluxbb&action=profile&id='.$cur_post['poster_id'].'">'.pun_htmlspecialchars($cur_post['username']).'</a>';
		else
		...
		if ($pun_config['o_show_user_info'] == '1')
		{
			...
			if ((($cur_post['email_setting'] == '0' && !$pun_user['is_guest']) || $pun_user['is_admmod']) && $pun_user['g_send_email'] == '1')
				...
				$user_contacts[] = '<span class="email"><a href="?module=fluxbb&action=misc&email='.$cur_post['poster_id'].'">'.$lang_common['Email'].'</a></span>';
		...
		{
			$user_info[] = '<dd><span><a href="?module=fluxbb&action=moderate&get_host='.$cur_post['id'].'" title="'.pun_htmlspecialchars($cur_post['poster_ip']).'">'.$lang_topic['IP address logged'].'</a></span></dd>';
	...
	{
		if (!$pun_user['is_guest'])
			$post_actions[] = '<li class="postreport"><span><a href="?module=fluxbb&action=misc&report='.$cur_post['id'].'">'.$lang_topic['Report'].'</a></span></li>';
		...
		if ($cur_topic['closed'] == '0')
		{
			if ($cur_post['poster_id'] == $pun_user['id'])
			{
				if ((($start_from + $post_count) == 1 && $pun_user['g_delete_topics'] == '1') || (($start_from + $post_count) > 1 && $pun_user['g_delete_posts'] == '1'))
					$post_actions[] = '<li class="postdelete"><span><a href="?module=fluxbb&action=delete&id='.$cur_post['id'].'">'.$lang_topic['Delete'].'</a></span></li>';
				if ($pun_user['g_edit_posts'] == '1')
					$post_actions[] = '<li class="postedit"><span><a href="?module=fluxbb&action=edit&id='.$cur_post['id'].'">'.$lang_topic['Edit'].'</a></span></li>';
			}

			if (($cur_topic['post_replies'] == '' && $pun_user['g_post_replies'] == '1') || $cur_topic['post_replies'] == '1')
				$post_actions[] = '<li class="postquote"><span><a href="?module=fluxbb&action=post&tid='.$id.'&amp;qid='.$cur_post['id'].'">'.$lang_topic['Quote'].'</a></span></li>';
		}
	}
	else
	{
...
<div id="quickpost" class="blockform">
	...
	<div class="box">
		<form id="quickpostform" method="post" action="?module=fluxbb&action=post&tid=<?php echo $id ?>" onsubmit="this.submit.disabled=true;if(process_form(this)){return true;}else{this.submit.disabled=false;return false;}">
			<div class="inform">
				<fieldset>
...
<textarea name="req_message" rows="7" cols="75" tabindex="<?php echo $cur_index++ ?>"></textarea></label>
						<ul class="bblinks">
							...
							<li><span><a href="?module=fluxbb&action=help#img" onclick="window.open(this.href); return false;"><?php echo $lang_common['img tag'] ?></a> <?php echo ($pun_config['p_message_bbcode'] == '1' && $pun_config['p_message_img_tag'] == '1') ? $lang_common['on'] : $lang_common['off']; ?></span></li>
							<li><span><a href="?module=fluxbb&action=help#smilies" onclick="window.open(this.href); return false;"><?php echo $lang_common['Smilies'] ?></a> <?php echo ($pun_config['o_smilies'] == '1') ? $lang_common['on'] : $lang_common['off']; ?></span></li>
						</ul>
					</div>
				</fieldset>
			</div>
