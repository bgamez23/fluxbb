...
?>
	<div class="blockform">
		...
		<div class="box">
			<form id="bans2" method="post" action="?module=fluxbb&action=admin_bans">
				<div class="inform">
							...
							<table class="aligntop" cellspacing="0">
								...
								<tr>
									...
									<td>
										...
										<span><?php echo $lang_admin_bans['IP help'] ?><?php if ($ban_user != '' && isset($user_id)) printf(' '.$lang_admin_bans['IP help link'], '<a href="?module=fluxbb&action=admin_users&ip_stats='.$user_id.'">'.$lang_admin_common['here'].'</a>') ?></span>
									</td>
								</tr>
								<tr>
...
{
	confirm_referrer('?module=fluxbb&action=admin_bans');
...
else if ($params->get('find_ban'))
{
	...
	$expire_after = $params->get('expire_after') ? pun_trim($params->get('expire_after')) : '';
	$expire_before = $params->get('expire_before') ? pun_trim($params->get('expire_before')) : '';
	$order_by = $params->get('order_by') && in_array($params->get('order_by'), array('username', 'ip', 'email', 'expire')) ? 'b.'.$params->get('order_by') : 'b.username';
	$direction = $params->get('direction') && $params->get('direction') == 'DESC' ? 'DESC' : 'ASC';
	...
	$paging_links = '<span class="pages-label">'.$lang_common['Pages'].' </span>'.paginate($num_pages, $p, '?module=fluxbb&action=admin_bans&find_ban=&amp;'.implode('&amp;', $query_str));
...
<div class="linkst">
	<div class="inbox crumbsplus">
		<ul class="crumbs">
			...
			<li><span>»&#160;</span><a href="?module=fluxbb&action=admin_bans"><?php echo $lang_admin_common['Bans'] ?></a></li>
...
<?php
	...
	if ($db->num_rows($result))
	{
		while ($ban_data = $db->fetch_assoc($result))
		{
			...
			$actions = '<a href="?module=fluxbb&action=admin_bans&edit_ban='.$ban_data['id'].'">'.$lang_admin_common['Edit'].'</a> | <a href="?module=fluxbb&action=admin_bans&del_ban='.$ban_data['id'].'">'.$lang_admin_common['Remove'].'</a>';
...
?>
				<tr>
					...
					<td class="tc6"><?php echo ($ban_data['ban_creator_username'] != '') ? '<a href="?module=fluxbb&action=profile&id='.$ban_data['ban_creator'].'">'.pun_htmlspecialchars($ban_data['ban_creator_username']).'</a>' : $lang_admin_bans['Unknown'] ?></td>
...
?>
	<div class="blockform">
		...
		<div class="box">
			<form id="bans" method="post" action="?module=fluxbb&action=admin_bans&act=more">
				<div class="inform">
					<fieldset>
		...
		<div class="box">
			<form id="find_bans" method="post" action="?module=fluxbb&action=admin_bans">
