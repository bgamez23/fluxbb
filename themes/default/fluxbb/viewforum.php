...
if (($cur_forum['post_topics'] == '' && $pun_user['g_post_topics'] == '1') || $cur_forum['post_topics'] == '1' || $is_admmod)
	$post_link = "\t\t\t".'<p class="postlink conr"><a href="?module=fluxbb&action=post&fid='.$id.'">'.$lang_forum['Post topic'].'</a></p>'."\n";
else
	$post_link = '';
...
$paging_links = '<span class="pages-label">'.$lang_common['Pages'].' </span>'.paginate($num_pages, $p, '?module=fluxbb&action=viewforum&id='.$id);

if ($pun_config['o_feed_type'] == '1')
	$page_head = array('feed' => '<link rel="alternate" type="application/rss+xml" href="?module=fluxbb&action=extern&act=feed&amp;fid='.$id.'&amp;type=rss" title="'.$lang_common['RSS forum feed'].'" />');
else if ($pun_config['o_feed_type'] == '2')
	$page_head = array('feed' => '<link rel="alternate" type="application/atom+xml" href="?module=fluxbb&action=extern&act=feed&amp;fid='.$id.'&amp;type=atom" title="'.$lang_common['Atom forum feed'].'" />');
...
if (!$pun_user['is_guest'])
{
	if ($pun_config['o_forum_subscriptions'] == '1')
	{
		if ($cur_forum['is_subscribed'])
			$forum_actions[] = '<span>'.$lang_forum['Is subscribed'].' - </span><a href="?module=fluxbb&action=misc&act=unsubscribe&amp;fid='.$id.'">'.$lang_forum['Unsubscribe'].'</a>';
		else
			$forum_actions[] = '<a href="?module=fluxbb&action=misc&act=subscribe&amp;fid='.$id.'">'.$lang_forum['Subscribe'].'</a>';
	}

	$forum_actions[] = '<a href="?module=fluxbb&action=misc&act=markforumread&amp;fid='.$id.'">'.$lang_common['Mark forum read'].'</a>';
}
...
<div class="linkst">
	<div class="inbox crumbsplus">
		<ul class="crumbs">
			...
			<li><span>»&#160;</span><strong><a href="?module=fluxbb&action=viewforum&id=<?php echo $id ?>"><?php echo pun_htmlspecialchars($cur_forum['forum_name']) ?></a></strong></li>
		</ul>
		<div class="pagepost">
...
if ($db->num_rows($result))
{
	...
	while ($cur_topic = $db->fetch_assoc($result))
	{
		...
		if (!$pun_user['is_guest'] && $cur_topic['last_post'] > $pun_user['last_visit'] && (!isset($tracked_topics['topics'][$cur_topic['id']]) || $tracked_topics['topics'][$cur_topic['id']] < $cur_topic['last_post']) && (!isset($tracked_topics['forums'][$id]) || $tracked_topics['forums'][$id] < $cur_topic['last_post']) && is_null($cur_topic['moved_to']))
		{
			...
			$subject_new_posts = '<span class="newtext">[ <a href="?module=fluxbb&action=viewtopic&id='.$cur_topic['id'].'&amp;act=new" title="'.$lang_common['New posts info'].'">'.$lang_common['New posts'].'</a> ]</span>';
		}
		else
		...
		if ($num_pages_topic > 1)
			$subject_multipage = '<span class="pagestext">[ '.paginate($num_pages_topic, -1, '?module=fluxbb&action=viewtopic&id='.$cur_topic['id']).' ]</span>';
		else
...
<?php echo (!empty($forum_actions) ? "\t\t".'<p class="subscribelink clearb" style="top:33px;">'.implode(' - ', $forum_actions).'</p>'."\n" : '') ?>
		<div class="clearer"></div>
	</div>
</div>
<?php
