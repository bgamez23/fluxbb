					...
					while ($cur_subscriber = $db->fetch_assoc($result))
					{
						...
						if (!isset($notification_emails[$cur_subscriber['language']]))
						{
							if (file_exists(PUN_ROOT.'lang/'.$cur_subscriber['language'].'/mail_templates/new_reply.tpl'))
							{
								...
								$mail_message = str_replace('<post_url>', '?module=fluxbb&action=viewtopic&pid='.$new_pid.'#p'.$new_pid, $mail_message);
								$mail_message = str_replace('<unsubscribe_url>', '?module=fluxbb&action=misc&act=unsubscribe&tid='.$tid, $mail_message);
								...
								$mail_message_full = str_replace('<post_url>', '?module=fluxbb&action=viewtopic&pid='.$new_pid.'#p'.$new_pid, $mail_message_full);
								$mail_message_full = str_replace('<unsubscribe_url>', '?module=fluxbb&action=misc&act=unsubscribe&tid='.$tid, $mail_message_full);
					...
					while ($cur_subscriber = $db->fetch_assoc($result))
					{
						...
						if (!isset($notification_emails[$cur_subscriber['language']]))
						{
							if (file_exists(PUN_ROOT.'lang/'.$cur_subscriber['language'].'/mail_templates/new_topic.tpl'))
							{
								...
								$mail_message = str_replace('<topic_url>', '?module=fluxbb&action=viewtopic&id='.$new_tid, $mail_message);
								$mail_message = str_replace('<unsubscribe_url>', '?module=fluxbb&action=misc&act=unsubscribe&fid='.$cur_posting['id'], $mail_message);
								$mail_message = str_replace('<board_mailer>', $pun_config['o_board_title'], $mail_message);
								...
								$mail_message_full = str_replace('<message>', $cleaned_message, $mail_message_full);
								$mail_message_full = str_replace('<topic_url>', '?module=fluxbb&action=viewtopic&id='.$new_tid, $mail_message_full);
								$mail_message_full = str_replace('<unsubscribe_url>', '?module=fluxbb&action=misc&act=unsubscribe&fid='.$cur_posting['id'], $mail_message_full);
								$mail_message_full = str_replace('<board_mailer>', $pun_config['o_board_title'], $mail_message_full);
...
else if ($fid)
{
	...
	$form = '<form id="post" method="post" action="?module=fluxbb&action=post&act=post&amp;fid='.$fid.'" onsubmit="return process_form(this)">';
}
else
	message($lang_common['Bad request']);
...
<div class="linkst">
	<div class="inbox">
		<ul class="crumbs">
			...
			<li><span>»&#160;</span><a href="?module=fluxbb&action=viewforum&id=<?php echo $cur_posting['id'] ?>"><?php echo pun_htmlspecialchars($cur_posting['forum_name']) ?></a></li>
<?php if (isset($cur_posting['subject'])): ?>			<li><span>»&#160;</span><a href="?module=fluxbb&action=viewtopic&id=<?php echo $tid ?>"><?php echo pun_htmlspecialchars($cur_posting['subject']) ?></a></li>
