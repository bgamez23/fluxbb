...
$action = $params->get('act') ? pun_trim($params->get('act')) : '';

if ($action == 'rebuild')
{
	$per_page = $params->get('i_per_page') ? intval($params->get('i_per_page')) : 0;
	$start_at = $params->get('i_start_at') ? intval($params->get('i_start_at')) : 0;
...
<?php
	...
	if ($end_at > 0)
	{
		...
		if ($db->num_rows($result) > 0)
			$query_str = '&act=rebuild&i_per_page='.$per_page.'&i_start_at='.$db->result($result);
	}

	$db->end_transaction();
	$db->close();

	exit('<script type="text/javascript">window.location="?module=fluxbb&action=admin_maintenance'.$query_str.'"</script><hr /><p>'.sprintf($lang_admin_maintenance['Javascript redirect failed'], '<a href="?module=fluxbb&action=admin_maintenance'.$query_str.'">'.$lang_admin_maintenance['Click here'].'</a>').'</p>');
}

if ($action == 'prune')
{
	$prune_from = pun_trim($params->get('prune_from'));
	$prune_sticky = intval($params->get('prune_sticky'));

	if ($params->get('prune_comply'))
	{
		confirm_referrer('?module=fluxbb&action=admin_maintenance');

		$prune_days = intval($params->get('prune_days'));
...
?>
	<div class="blockform">
		...
		<div class="box">
			<form method="post" action="?module=fluxbb&action=admin_maintenance">
				<div class="inform">
...
?>
	<div class="blockform">
		...
		<div class="box">
			<form method="post" action="?module=fluxbb&action=admin_maintenance">
				<div class="inform">
					<input type="hidden" name="act" value="rebuild" />
					<fieldset>
						<legend><?php echo $lang_admin_maintenance['Rebuild index subhead'] ?></legend>
						<div class="infldset">
							<p><?php printf($lang_admin_maintenance['Rebuild index info'], '<a href="?module=fluxbb&action=admin_options#maintenance">'.$lang_admin_common['Maintenance mode'].'</a>') ?></p>
			...
			<form method="post" action="?module=fluxbb&action=admin_maintenance" onsubmit="return process_form(this)">
				<div class="inform">
					<input type="hidden" name="act" value="prune" />
					<fieldset>
...
?>
							...
							<p class="topspace"><?php printf($lang_admin_maintenance['Prune info'], '<a href="?module=fluxbb&action=admin_options#maintenance">'.$lang_admin_common['Maintenance mode'].'</a>') ?></p>
